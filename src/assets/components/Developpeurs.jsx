export function Developpeurs({ image, nom, fonction, niveau, chef, linkedin, whatsapp }) {
    return (
        <div className="relative rounded-xl bg-white shadow-lg p-6 w-80 border border-slate-200 hover:shadow-xl transition-all duration-300 hover:-translate-y-1 group">
            {chef && (
                <div className="absolute -top-3 left-1/2 transform -translate-x-1/2">
                    <span className="bg-gradient-to-r from-slate-500 to-slate-600 text-white text-xs font-bold px-3 py-1 rounded-full shadow-md">
                        {chef}
                    </span>
                </div>
            )}

            <div className="flex flex-col items-center space-y-4">
                <div className="relative">
                    <img 
                        src={image} 
                        alt={nom}
                        className="w-24 h-24 rounded-full border-4 border-white shadow-md object-cover border-blue-200 transition-colors duration-300"
                    />
                </div>

                <div className="text-center">
                    <h2 className="text-xl font-bold text-slate-800">{nom}</h2>
                    <p className="text-slate-600 text-sm mt-1">{niveau}</p>

                    {/* Ligne avec les icônes et la fonction */}
                    <div className="flex items-center justify-center gap-3 mt-3">
                        {/* 🔗 Icône LinkedIn (affichée seulement si le lien est fourni) */}
                        {linkedin && (
                            <a 
                                href={linkedin} 
                                target="_blank" 
                                rel="noopener noreferrer"
                                className="hover:scale-110 transition-transform duration-300 p-2 rounded-full bg-blue-50 hover:bg-blue-100"
                                title="Profil LinkedIn"
                            >
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 72 72">
                                    <rect width="72" height="72" fill="#117EB8" rx="4"></rect>
                                    <path fill="#FFF" d="M13.139 27.848h9.623V58.81h-9.623V27.848zm4.813-15.391c3.077 0 5.577 2.5 5.577 5.577 0 3.08-2.5 5.581-5.577 5.581a5.58 5.58 0 1 1 0-11.158zm10.846 15.39h9.23v4.231h.128c1.285-2.434 4.424-5 9.105-5 9.744 0 11.544 6.413 11.544 14.75V58.81h-9.617V43.753c0-3.59-.066-8.209-5-8.209-5.007 0-5.776 3.911-5.776 7.95V58.81h-9.615V27.848z"></path>
                                </svg>
                            </a>
                        )}

                        {/* 🧑‍💻 Fonction */}
                        <span className="px-3 py-1 bg-gradient-to-r from-slate-400 to-slate-900 text-white rounded-full text-xs font-medium shadow-sm">
                            {fonction}
                        </span>

                        {/* 🔗 Icône WhatsApp (affichée seulement si le numéro est fourni) */}
                        {whatsapp && (
                            <a 
                                href={`https://wa.me/+225${whatsapp}`} 
                                target="_blank" 
                                rel="noopener noreferrer"
                                className="hover:scale-110 transition-transform duration-300 p-1 rounded-full bg-green-50 hover:bg-green-100"
                                title="Contacter sur WhatsApp"
                            >
                                <svg xmlns="http://www.w3.org/2000/svg" width="35" height="35" viewBox="0 0 13333.33 13333.33">
                                    <path fill="#25D366" d="M4368.28 2476.11l4596.93 0c1027.92,0 1867.22,841.16 1867.22,1867.37l0 4644.48c0,1028.14 -839.3,1869.26 -1867.22,1869.26l-4596.93 0c-1028.07,0 -1867.35,-841.12 -1867.35,-1869.26l0 -4644.48c0,-1026.2 839.28,-1867.37 1867.35,-1867.37z"></path>
                                    <path fill="#ffffffff" d="M6716.35 8886.92c-469.18,0 -904.19,-141.18 -1270.46,-379.6l-884.95 280.41 286.13 -852.62c-276.57,-373.83 -440.65,-843.08 -440.65,-1344.72 0,-74.39 5.7,-144.96 15.26,-219.35 112.55,-1161.62 1100.61,-2069.53 2294.67,-2069.53 1213.07,0 2210.65,932.72 2304.05,2111.5 3.84,61.03 3.84,122.06 3.84,177.39 0,1264.61 -1035.6,2296.54 -2307.89,2296.54z"></path>
                                    <path fill="#25D366" d="M7517.45 6901.28c-66.9,-17.17 -108.75,-32.43 -156.46,38.16 -68.72,108.72 -213.67,347.14 -354.76,276.57 -70.55,-32.43 -286.04,-103.01 -543.57,-331.9 -200.32,-179.3 -337.58,-394.83 -373.95,-459.67 -59.04,-110.63 127.76,-226.99 196.48,-366.22 22.85,-45.79 13.34,-83.93 -5.67,-116.35 -13.34,-32.43 -148.78,-364.31 -205.99,-495.92 -55.38,-135.44 -112.59,-112.54 -148.78,-112.54 -373.96,0 -549.42,265.13 -549.42,637.06 0,74.39 15.35,154.52 38.2,225.09 66.71,238.43 215.49,431.07 238.52,463.5 32.35,45.77 463.52,740.07 1148.19,1007.14 688.51,267 688.51,177.36 810.6,167.8 120.27,-9.49 396.8,-158.32 454.01,-318.54 55.38,-154.48 55.38,-289.93 36.19,-318.52 -22.85,-40.05 -583.59,-293.74 -583.59,-295.65z"></path>
                                </svg>
                            </a>
                        )}
                    </div>
                </div>
            </div>
        </div>
    );
}